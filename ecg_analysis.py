...
def read_file(filename):
    ...
    """
    logging.info("Analyzing ECG trace from {}".format(filename))
    data_file = open("{}.csv".format(filename), 'r')
    in_lines = data_file.readlines()
    data_file.close()
    return in_lines, filename
    ...
    for c, ele in enumerate(data):
        for d, value in enumerate(ele):
            if any(x.isalpha() for x in value) or value == '':
                idx.append(c)
                logging.error("A data point with an index of {}"
                              " was skipped due to bad data".format(c))
                break
...
def voltage_range(voltages, filename):
    ...
    for ele in voltages:
        if ele > 300 or ele < -300:
            logging.warning("{} has voltage values that"
                            " exceed the normal range".format(filename))
            break
...
def voltage_extremes(voltage):
    ...
    """
    logging.info("Calculating min and max voltage values")
...
def peak_find(time, voltage):
    ...
    Returns:
        ...
        log: an info log that the times of the beats are being
            determined
    """
    logging.info("Calculating the times when beats occur")
...
def num_beats(time_of_beats):
    ...
    Returns:
        ...
        log: an info log that the number of beats is being
            calculated
    """
    logging.info("Calculating the number of beats")
...
def avg_HR(beat_num, duration):
    ...
    Returns:
        ...
        log: an info log that the average heart rate is being
            calculated
    """
    logging.info("Calculating the average heart rate in bpm")
...
def driver():
    ...
    """
    in_lines, filename = read_file("test_data11")
    ...
    voltage_range(volt, filename)
    filtered = BandPassFilter(volt)

    duration = duration_calc(time)
    extremes = voltage_extremes(volt)
    time_beats = peak_find(time, filtered)
    beat_num = num_beats(time_beats)
    mean_HR = avg_HR(beat_num, duration)

    metrics = patient_dict(duration, extremes, beat_num,
                           mean_HR, time_beats)
    out_filename = output_filename(filename)
    out_file = output_json(out_filename, metrics)


if __name__ == "__main__":
    logging.basicConfig(filename="ecg_analysis_log.log", filemode='w',
                        level=logging.INFO)
    driver()
